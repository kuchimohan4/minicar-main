package com.Deka.MovieSchedule.MovieScheduleService.Entity;
...
//	@Future(message = "date should be in future")
	...
	public ParkingSlot(@Size(max = 3, message = "INVALID PARKING SLOT") String slot,
			@Size(min = 3, message = "Movie title Should be minimum of size 3") String movieTitle,
			@Pattern(regexp = "\\d{2}-\\d{2}-\\d{4}", message = "Date should be in dd-MM-yyyy format") String date,
			String playTime, boolean avliable) {
		super();
		this.slot = slot;
		this.movieTitle = movieTitle;
		this.date = date;
